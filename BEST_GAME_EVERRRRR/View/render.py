    ...
    def render(self, data):
        ...
        for i in range(_map_size):
            for j in range(_map_size):
                _cycle_ground = Image.open(f"Govnogame/res/{_ground[random.randint(0, 3)]}").resize((_one_point_size, _one_point_size))
                _map.paste(_cycle_ground, (i*_one_point_size, j*_one_point_size))

        for i in range(len(_user_buildings)):
            _building_texture = Image.open(f'Govnogame/res/{_user_buildings[i]["texture"]}').resize((_one_point_size*_user_buildings[i]["size"], _one_point_size*_user_buildings[i]["size"]))
